    ...
    public class ProjectController : Controller
    {
        ...
        public ActionResult My()
        {
            ...
            if (user != null)
            {
                if (user.GroupId == 1)
                {
                    var myProject = new ProjectDAO().Get(0, "", user.Id, "", 0, 0, "", "", "", 2, 0, 0);
                    if (myProject.Count > 0)
                        return Redirect("/Project/Detail/" + myProject[0].Id);
                    else
                        return View();
                }
                else
        ...
        public void ExportExcel(string name = "", string student = "", string lecturer = "", long projectTypeId = 0, int year = 0, string classId = "", int pointStatus = 2)
        {
            User user = (User)Session["USER_SESSION"];

            ProjectDAO projectDAO = new ProjectDAO();
            List<Project> projects = projectDAO.Get(0, name, student, lecturer, projectTypeId, year, "", user.BranchId, classId, pointStatus, 0, 0);
